            ...
            <div className="card container w-50 my-2 p-0">
                ...
                <div className="card-body d-flex py-3">
                    ...
                    <textarea type="text" className="form-control border border-white card container" 
                        ...
                        onChange={e => this.setState({ caption: e.target.value }, () => { if (this.state.caption !== "") { this.setState({ canSubmit: false }); } else { this.setState({ canSubmit: false }); } })} />
                    <input type="file" multiple className="form-control-file" ref={btn => this.fileInput = btn}
                        onChange={e => this.setState({ files: e.target.files }, () => { if (this.state.files) { this.setState({ canSubmit: true }); } else { this.setState({ canSubmit: true }); } })} />
                </div>
